					...
					if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
					{
						...
						$invoice_state = $invoice->id_state ? new State($invoice->id_state) : false;
                                                ...
                                                if($carrier->name == "Packlink"){
                                                    ...
                                                    // $totalNoShipping = (float)str_replace(' ', '', $template_vars['{total_paid}']) - (float)str_replace(' ', '', $template_vars['{total_shipping}']);
                                                    $totalNoShipping = $total_paid - $total_shipping_tax_incl;
                                                    ...
                                                    foreach ($dom as $shipper){
                                                        foreach($shipper as $service){ 
                                                            if((int)$service->service_id == $id_carrier_packlink){
                                                                $carrier_packlink_name = (string)$shipper->getName()." ".((string)$service->service);
                                                                $break = true;
                                                            }
                                                            if($break) break;
                                                        }
                                                    ...
                                                    $carrier_packlink_name = $carrier->name." / ".$carrier_packlink_name;
                                                } else {
                                                    $carrier_packlink_name = $virtual_product ? Tools::displayError('No carrier') : $carrier->name;
