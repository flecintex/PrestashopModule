...
function callback(data){
    ...
    pl_inf = "";
    delCookie(md5('upSeedPL'+getCookie(md5('upSeed'))));
    delCookie(md5('upSeed'));    
...
function delivery_option_title_fnt(){
    ...
    if(reload){
        ...
        if(pl_inf == null || pl_inf == ""){
            pl_inf = "";
            try { delCookie(md5('upSeedPL'+getCookie(md5('upSeed'))));} catch (e) {}
            try { delCookie(md5('upSeed'));} catch (e) {}
            return "";
        }
...
function dcr(){var aux = decrypt(getCookie(md5('upSeedPL'+getCookie(md5('upSeed')))), getCookie(md5('upSeed')));return aux.substr(0, aux.toString().length-1).split("|");}
...
function fullDateMin(){
    var aux = new Date();
    
    var ret  = aux.getFullYear();
        ret += (parseInt(aux.getMonth())+1) < 10 ? "0" + (parseInt(aux.getMonth())+1) : (""+(parseInt(aux.getMonth())+1));
        ret += aux.getDate() < 10 ? "0" + aux.getDate() : (""+aux.getDate());
        ret += aux.getHours() < 10 ? "0" + aux.getHours() : (""+aux.getHours());
        ret += aux.getMinutes() < 10 ? "0" + aux.getMinutes() : (""+aux.getMinutes());
